#!/usr/bin/env python3
"""Generate two-column paginated HTML v4 - optimized page distribution.

Changes from v3:
- P1: Cover + FULL Introduction (all 3 paragraphs, margin-top:20mm)
- P2: All Methods 2.1-2.7 (no intro text)
- Discussion merged from 3→2 pages
- References split into 2 pages (v3 had hidden overflow!)
- Per-page line-height tuning
Target: 13 pages
"""
...
def page1():
    """Cover + FULL Introduction (all 3 paragraphs, flexbox columns)."""
    intro_left = ("Renal malignancies are among the ten most common cancers worldwide, "
        "accounting for approximately 2% of all human tumors [1]. According to global "
        "cancer statistics in 2020, about 431,288 new cases of renal cancer were diagnosed, "
        "with approximately 179,368 related deaths [2]. Clear cell renal cell carcinoma "
        "(ccRCC) is the most prevalent subtype of renal cell carcinoma, representing nearly "
        "75% of cases [3]. Although radical or partial nephrectomy remains the primary "
        "treatment for localized ccRCC, approximately one-third of patients eventually "
        "develop distant metastases, resulting in limited therapeutic options and poor "
        "prognosis [4]. Despite advances in targeted therapies, drug resistance has emerged "
        "as a major clinical challenge, highlighting the urgent need to identify novel "
        "biomarkers and therapeutic strategies for ccRCC."
        "</p><p>With the rapid development of next-generation sequencing technologies, "
        "high-throughput transcriptomic analyses have been widely applied in cancer "
        "research [5]. Publicly available databases, such as The Cancer Genome Atlas (TCGA) "
        "and the Gene Expression Omnibus (GEO), provide large-scale gene expression data "
        "that facilitate systematic bioinformatic investigations into cancer-related "
        "molecular mechanisms [6]. Although numerous bioinformatic studies have explored "
        "the molecular characteristics of ccRCC, most have focused on identifying diagnostic "
        "or prognostic biomarkers, while relatively few have investigated potential "
        "therapeutic agents targeting these key genes.")

    intro_right = ("In the present study, integrated bioinformatic analyses were performed "
        "to identify key genes associated with ccRCC progression and to explore potential "
        "therapeutic compounds. Differentially expressed genes were identified using "
        "multiple GEO datasets and subsequently validated in the TCGA database. Candidate "
        "drugs targeting the key genes were further screened, and molecular docking analyses "
        "were conducted to evaluate their binding potential. Overall, this study aims to "
        "provide new insights into the diagnosis and targeted treatment of ccRCC.")
...
def page2():
    """Methods 2.1-2.7 (all methods, no intro text)."""
    return f'''
<div class="page">
{header()}
...
def page3():
    """Table 1 + Results 3.1 + Figs 1+2 + Results 3.2 + Fig 3."""
    return f'''
<div class="page">
{header()}
    <div class="page-content two-column" style="line-height:1.25;">
...
def page5():
    """GEPIA2 validation text + Fig 5."""
    return f'''
<div class="page">
{header()}
    <div class="page-content two-column" style="line-height:1.4;">
        <p class="no-indent">Validation in GEPIA2 revealed that the mRNA expression levels of FN1, TIMP1, VEGFA, TGFB1, PECAM1, and CAV1 were significantly up-regulated in ccRCC tissues compared with normal tissues (Figure 5). Further analysis of these genes across different tumor stages using GEPIA2 and UALCAN indicated that only TGFB1 and TIMP1 expression levels were consistently correlated with ccRCC progression at both mRNA (Figure 6A) and protein levels (Figure 6B), suggesting their potential roles in promoting tumor progression. Kaplan&ndash;Meier survival analysis demonstrated that high expression of TGFB1 and TIMP1 was associated with poorer overall survival in ccRCC patients (Figure 6C).</p>
        {fig(5, FIG_CAP[5])}
    </div>
{footer(5)}
</div>'''
...
def page8():
    """ROC + Fig 8 + Pearson + TIMP1 enrichment."""
    return f'''
<div class="page">
{header()}
    <div class="page-content two-column" style="line-height:1.4;">
...
def page10():
    """Results 3.5 P2 + Fig 10 + Discussion P1-P2."""
    return f'''
<div class="page">
{header()}
    <div class="page-content two-column" style="line-height:1.4;">
...
def page11():
    """Discussion P3-P5 (PPI hub genes + TGFB1 + TIMP1)."""
    return f'''
<div class="page">
{header()}
    <div class="page-content two-column" style="line-height:1.4;">
        <p class="no-indent">After the construction of the PPI network of shared DEGs, the top 10 hub genes were identified. The expressional differences of 9 hub genes between ccRCC tissues and normal tissues were consistent with the results of the GEPIA2 database. A total of 6 genes including FN1, TIMP1, VEGFA, TGFB1, PECAM1, and CAV1 were up-regulated in ccRCC, in which TGFB1 and TIMP1 were found to be correlated with the progression and prognosis of ccRCC. In the TCGA database, we further proved that TGFB1 and TIMP1 were highly expressed in the higher TNM stage of ccRCC and showed good expression specificity in ccRCC, which suggested that TGFB1 and TIMP1 may be the potential biomarkers of ccRCC.</p>
...
def page13():
    """References 1-40 (split into 2 pages: first ~22 refs)."""
    refs_html = "\n            ".join(f'<div>{r}</div>' for r in REFS[:22])
    return f'''
<div class="page">
{header()}
    <div class="page-content two-column" style="line-height:1.4;">
...
if __name__ == "__main__":
    ...
    for path in [out1, out2]:
        with open(path, "w", encoding="utf-8") as f:
            f.write(html)
    print(f"Generated {len(html):,} bytes, 14 pages")
    print(f"  -> {out1}")
    print(f"  -> {out2}")
