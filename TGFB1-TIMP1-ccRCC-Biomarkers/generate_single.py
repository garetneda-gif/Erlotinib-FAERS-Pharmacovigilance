#!/usr/bin/env python3
"""Generate single-column continuous HTML for TGFB1-TIMP1-ccRCC-Biomarkers paper.

Output: single-column-TGFB1-TIMP1-ccRCC-Biomarkers.html
Format: MedBA Medicine 单栏连续版（在线预览用，无分页）
"""

import os, sys, re, urllib.parse
sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))
from generate_v2 import IMG_BASE, FIG_CAP, REFS

OUTPUT_DIR = os.path.dirname(os.path.abspath(__file__))

# ============================================================
# Single-column CSS (from journal-typesetting skill template)
# ============================================================
SINGLE_CSS = """
        * { box-sizing: border-box; }
        body {
            ...
            font-size: 10pt; line-height: 1.38; color: #000;
        }
        a {
        ...
        h1 {
            ...
            text-transform: uppercase; margin: 8mm 0 3mm;
        ...
        p { margin: 0 0 3mm; text-indent: 1em; text-align: justify; }
        p.no-indent { text-indent: 0; }
        figure {
            margin: 6mm 0; width: 100%; text-align: center;
        }
        figure img {
            width: 100%; height: auto; max-width: 100%;
            max-height: 120mm; object-fit: contain; display: block;
            margin: 0 auto;
        }
        figcaption {
            font-size: 8.5pt; margin-top: 2mm;
            text-align: left; line-height: 1.3; color: #222;
        }
        figcaption .fig-label { font-weight: bold; color: #005a8c; }
        .side-by-side-figures {
            display: flex; gap: 5mm; margin: 6mm 0;
        }
        .side-by-side-figures figure { flex: 1; margin: 0; min-width: 0; }
        .table-wrapper { margin: 6mm 0; width: 100%; }
        .table-caption {
            font-size: 8.5pt; margin-bottom: 2mm;
        ...
        thead { border-bottom: 0.75pt solid #000; }
        th {
            padding: 2mm 2mm; text-align: left;
            font-weight: bold; font-size: 8pt;
        }
        td { padding: 1.5mm 2mm; text-align: left; font-size: 8pt; }
        tbody tr:nth-child(even) { background: #f8f9fa; }
        .references { margin-top: 6mm; }
        .references .ref-item {
            margin-bottom: 2.5mm; font-size: 8pt; line-height: 1.3;
            padding-left: 1.5em; text-indent: -1.5em;
        }
        .ref-meta {
            font-size: 7.5pt; margin-top: 0.5mm;
            padding-left: 1.5em;
        }
        .ref-meta a { color: #005a8c; text-decoration: none; }
        @media print {
            body { background: #fff; padding: 0; }
            #page { box-shadow: none; padding: 20mm; max-width: none; }
        }
"""

# ============================================================
# Reference link metadata builder
# ============================================================
def extract_pmid(ref_html):
    """Extract PMID from PubMed URL in reference."""
    m = re.search(r'pubmed\.ncbi\.nlm\.nih\.gov/(\d+)', ref_html)
    return m.group(1) if m else None

def extract_doi(ref_html):
    """Extract DOI from doi.org URL in reference."""
    m = re.search(r'doi\.org/(10\.[^"<\s]+)', ref_html)
    return m.group(1) if m else None

def extract_title(ref_html):
    """Extract title text from bold+link in reference."""
    m = re.search(r'<a[^>]*>([^<]+)</a>', ref_html)
    return m.group(1) if m else ""

def build_ref_meta(ref_html):
    """Build metadata line: PubMed | Google Scholar | Crossref."""
    pmid = extract_pmid(ref_html)
    doi = extract_doi(ref_html)
    title = extract_title(ref_html)

    links = []
    if pmid:
        links.append(f'<a href="https://pubmed.ncbi.nlm.nih.gov/{pmid}/">PubMed</a>')

    # Google Scholar always
    q = urllib.parse.quote_plus(title[:80]) if title else ""
    if q:
        links.append(f'<a href="https://scholar.google.com/scholar?q={q}">Google Scholar</a>')

    if doi:
        links.append(f'<a href="https://doi.org/{doi}">Crossref</a>')

    return " | ".join(links)

def format_ref(ref_html):
    """Format reference with metadata row for single-column version."""
    meta = build_ref_meta(ref_html)
    result = f'<div class="ref-item">{ref_html}</div>'
    if meta:
...
# ============================================================
def build_html():
    refs_html = "\n            ".join(format_ref(r) for r in REFS)
...
<div id="page">
    ...
    <div style="display:flex;justify-content:space-between;align-items:flex-end;font-size:7pt;border-bottom:0.5pt solid #000;padding-bottom:1mm;margin-bottom:2mm;">
        <div>February 2026 <span style="margin-left:2em;">DOI: 10.65079/xxx</span></div>
    ...
    <!-- ===== TITLE & AUTHORS ===== -->
    <div style="font-size:18pt;font-weight:bold;margin-bottom:8mm;line-height:1.2;">Identification of TGFB1 and TIMP1 as the Biomarkers of Clear Cell Renal Cell Carcinoma by Bioinformatic Methods</div>
    <div style="font-size:11pt;margin-bottom:6mm;">Lifang Zhou<sup>1</sup>, Nana Li<sup>1</sup>, Yiying Zhang<sup>1</sup>, Hongling Hu<sup>1</sup>, Hongjian Zhang<sup>1,#</sup></div>

    <!-- ===== AFFILIATIONS ===== -->
    <div style="font-size:8pt;line-height:1.3;margin-bottom:4mm;">
        <p style="margin:0 0 2mm;text-indent:0;"><span style="vertical-align:super;font-size:0.7em;">1</span> Department of Cancer, Zhongshan Hospital of Xiamen University, Xiamen, China.</p>
        <p style="margin:0 0 2mm;text-indent:0;"><span style="font-weight:bold;">Correspondence: </span>Hongjian Zhang, MD, Zhongshan Hospital of Xiamen University, Xiamen, 361000, China. Email: zhanghongjian11021@163.com</p>
        <p style="margin:0;text-indent:0;"><span style="font-weight:bold;">Funding information: </span>This study was supported by the Xiamen Medical and Health Guidance Project, China (Grant NO. 3502Z20254ZD1124).</p>
    </div>

    <!-- ===== ABSTRACT ===== -->
    <div id="abstract-box" style="width:100%;border:1pt solid #b8b8b8;border-left:3pt solid #005a8c;padding:6mm 7mm;position:relative;box-sizing:border-box;overflow-wrap:break-word;box-shadow:0 0 15px rgba(0,0,0,0.15);background-color:#f8f9fa;margin-bottom:8mm;">
    ...
    <h2>2.1 Microarray data collection and identification of shared DEGs</h2>
    ...
    <h2>2.2 Enrichment analysis of shared DEGs</h2>
    ...
    <h2>2.3 The PPI network construction and hub genes selected</h2>
    ...
    <h2>2.4 Expression and prognosis analysis of hub genes</h2>
    ...
    <h2>2.5 Validation of key genes in the TCGA database</h2>
    ...
    <h2>2.6 Molecular docking of active drugs and targets</h2>
    ...
    <!-- ===== 3 RESULTS ===== -->
    <h1>3 RESULTS</h1>
    <h2>3.1 Identification of DEGs</h2>
    ...
    <h2>3.2 Gene enrichment analysis</h2>
    ...
    <h2>3.3 Construction of PPI and identification of key genes</h2>
    ...
    <h2>3.4 Verification of TGFB1 and TIMP1 in the TCGA database</h2>
    ...
    <h2>3.5 Molecular docking of active drugs and targets</h2>
    ...
    <!-- ===== FOOTER ===== -->
    <div style="margin-top:10mm;padding-top:3mm;border-top:0.5pt solid #000;font-size:7pt;text-align:center;color:#666;">
        https://medbam.org
    </div>
</div>
</body>
...
if __name__ == "__main__":
    html = build_html()
    out = os.path.join(OUTPUT_DIR, "single-column-TGFB1-TIMP1-ccRCC-Biomarkers.html")
